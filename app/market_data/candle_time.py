"""Generic, provider-agnostic candle-time primitives: UTC epoch-aligned
timeframe boundaries and closed-vs-forming candle classification.

Relocated here (MT5 Price Authority Stage A corrective review, dependency-
direction finding) from what was originally ``app.technical.alignment``'s
own copy: these five functions carry zero Technical-domain concept (no
snapshot, engine, analyst or supervisor knowledge) - they are pure functions
of ``OHLCVCandle``/``Timeframe``/``as_of`` only, so market-data ingestion
code (e.g. ``app.market_data.providers.mt5``) can depend on them without
creating an upward dependency onto the Technical application/domain
contour. ``app.technical.alignment`` re-exports these same names unchanged
for its own existing call sites (``contiguous_tail`` - genuinely Stage-3-
specific rolling-window logic - stays there, not moved).
