...
class SetupBlockReason(StrEnum):
    ...
    already-authorized Judge thesis. ``INVALID_STOP_SIDE`` applies when the
    tick-normalized MT5-native structural stop sits on the wrong side of the
    resolved entry price. ``SHARED_FACT_UNAVAILABLE`` applies to the two
    facts every structure-capable family equally depends on: MT5 symbol
    facts (missing, or invalid tick economics) and the M15 market-structure
    block itself (missing, or below approved quality) - never fabricated,
    never treated as a per-family reason. ``BROKER_STOP_DISTANCE`` applies
    when the tick-normalized stop's distance from entry is smaller than the
    broker's own reported minimum (``trade_stops_level``) - the stop is
    never widened to satisfy this; the setup is rejected instead.
    """
