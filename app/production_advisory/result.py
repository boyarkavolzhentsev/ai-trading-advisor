...
class ProductionAdvisoryCycleResult(DomainModel):
    ...
    identifier (``SymbolMapping.logical_symbol``), fed to no provider.
    Deliberately no second, provider-native cycle-level symbol field here
    (MT5 Price Authority Stage C removal): once Flow and Technical
    intentionally read from different providers, no single such field could
    honestly describe "the" analytical evidence source any more. The MT5
    broker-facing symbol is never reported here either - it lives only on
    each issued ``FinalRecommendation``/``RecommendationDTO``, since a cycle
    with zero recommendations has no single broker symbol to report at this
    level.
    """
