"""Stage 6B must not modify any legacy Stage-0 contract or
``app/judge/__init__.py``.

Pins each file's exact content via a SHA-256 hash computed against the
approved Stage 6B baseline (HEAD ``be78ea6e8ee391375fd98ec4283f201fa47d6418``)
- any byte-level change to these files fails this test. Mirrors
``tests/test_strategy_router_legacy_untouched.py`` one stage over -
``README.md`` is deliberately not pinned here either, for the same reason.
"""
...
def test_file_byte_identical_to_pre_stage_6b_baseline(relative_path, expected_hash) -> None:
    content = (REPO_ROOT / relative_path).read_bytes()
    actual_hash = hashlib.sha256(content).hexdigest()
    assert actual_hash == expected_hash, f"{relative_path} was modified since the Stage 6B baseline"
