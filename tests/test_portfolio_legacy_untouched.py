"""Stage 8 must not modify any legacy Stage-0 contract, nor any Stage 5/6/7
production file.

Pins each file's exact content via a SHA-256 hash computed against the
approved Stage 8 baseline (HEAD ``bdf782f16baf2eaf5f175ba4b3a2e7d3c6a65f08``)
- any byte-level change to these files fails this test. Mirrors
``tests/test_risk_gate_legacy_untouched.py`` one stage over.
``app/core/config/trading_cycle.py`` is deliberately not pinned here: its
extension with ``portfolio_risk_limit_percent`` is the one explicitly
...
EXPECTED_SHA256 = {
    ...
    # Stage 8's own pre-existing package marker - must remain the original stub
...
def test_file_byte_identical_to_pre_stage_8_baseline(relative_path, expected_hash) -> None:
    content = (REPO_ROOT / relative_path).read_bytes()
    actual_hash = hashlib.sha256(content).hexdigest()
    assert actual_hash == expected_hash, f"{relative_path} was modified since the Stage 8 baseline"
